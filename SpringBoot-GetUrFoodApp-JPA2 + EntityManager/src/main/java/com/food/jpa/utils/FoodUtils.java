...
public class FoodUtils {
	...
	public ResponseEntity<String> doSignOn(String username, String password) {
		...
		try {
			/*
			 * signOn = foodService.userSignOn(username, password); if
			 * (!StringUtils.isEmpty(signOn) && signOn.contains("fail")) { signOnResponse =
			 * new ResponseEntity<String>(signOn, HttpStatus.BAD_REQUEST); } else {
